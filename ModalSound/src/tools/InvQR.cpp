...
public:
  ...
  SparsePlusLowRank(const SparseData& M, const SparseData& K, const Eigen::MatrixXd& R) : M(M), K(K), R(R), cg(M.getMap()) {
  ...
  namespace internal {
    ...
    : generic_product_impl_base<SparsePlusLowRank, Rhs, generic_product_impl<SparsePlusLowRank, Rhs>> {
      ...
      static void scaleAndAddTo(Dest& dst, const SparsePlusLowRank& lhs, const Rhs& rhs, const Scalar& alpha) {
        ...
        dst.noalias() += alpha * lhs.cg.solve(lhs.K.getMap().selfadjointView<Eigen::Upper>() * rhs);
...
Eigen::VectorXd invQR(const SparsePlusLowRank& A, int k) {
  ...
  for (int iter=0; iter<maxIters; iter++) {
    ...
    for (int i=0; i<k; i++) {
      S(i) = Z.col(i).dot(A.K.getMap().selfadjointView<Eigen::Upper>() * Z.col(i)) /
      Z.col(i).dot(A.M.getMap().selfadjointView<Eigen::Upper>() * Z.col(i));
    }
    std::cout << "iter " << iter << ": " << S.transpose() << std::endl;
  }
